...
class glassHousesTests: XCTestCase {
    ...
    func testNewsArticleParsing() {
        ...
        let persistentContainer: NSPersistentContainer = {
            let persistentContainer = NSPersistentContainer(name: "glassHouses")
            let memoryDescription = NSPersistentStoreDescription()
            memoryDescription.type = NSInMemoryStoreType
            persistentContainer.persistentStoreDescriptions = [memoryDescription]
            persistentContainer.loadPersistentStores(completionHandler: { (description, error) in
                if let error = error { print(error) }
            })
            return persistentContainer
        }()
        let legislator = Legislator(context: persistentContainer.viewContext)
        ...
        let allNewsArticles = jsonDictionaries!.flatMap({Article.fromJSON($0, legislator: legislator, into: persistentContainer.viewContext)})
    ...
    func testLegislationParsing() {
        ...
        let allLegislation = allJSONDict.flatMap({Legislation.fromJSON($0, into: persistentContainer.viewContext)})
    ...
    func testLegislatorParsing() {
        ...
        let allJSONDict = allJSONAny.flatMap({$0 as? [String: Any]})
        XCTAssertTrue(urls.count == allJSONDict.count, "failed to cast all json objects from any to dictionary")
        let allLegislators = allJSONDict.flatMap({Legislator.fromJSON($0, into: persistentContainer.viewContext)})
